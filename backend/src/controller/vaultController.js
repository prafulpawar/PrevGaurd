...
const AUTH_TAG_LENGTH = 16; // Standard Auth Tag length for AES-GCM
...
module.exports.loginVaultController = async (req, res) => {
    try {
        ...
        // IMPORTANT: Only confirm successful login. Data fetching is a separate step.
...
module.exports.folderVaultController = async (req, res) => {
     try {
        ...
        // Return the created folder object (without items)
        return res.status(201).json({
            message: "Folder Created Successfully",
            folder: {
                _id: newFolder._id,
                name: newFolder.name,
                items: [] // Send empty items array consistent with getVaultData
            }
        });
