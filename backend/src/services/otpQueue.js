...
async function otpcreateChannel() {
    ...
    console.log('✅ RabbitMQ Channel Created for OTP');
    return channel;
}
    ...
    try {
        ...
        channel.consume(queueName, async (msg) => {
            ...
            try {
                ...
                if (!redisData) {
                    logger.warn(`[OTP Worker] OTP data expired or not found in Redis for ${email} (RequestID: ${requestId})`);
                    await redis.set(`status:${requestId}`, JSON.stringify({ status: "expired", message: "OTP has expired or is invalid." }), "EX", 300); 
                    return channel.ack(msg);
                }
