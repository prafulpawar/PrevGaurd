...
const Button = ({
  ...
  const variantStyles = {
    primary: `
      ...
      ${disabled || isLoading ? 'bg-indigo-400 hover:bg-indigo-400' : ''}
    `,
    secondary: `
      text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:ring-indigo-500
       ${disabled || isLoading ? 'bg-indigo-50 hover:bg-indigo-50 text-indigo-400' : ''}
    `,
    link: `
      ...
      ${disabled || isLoading ? 'text-indigo-300 hover:text-indigo-300' : ''}
    `,
    ...
    >
      {isLoading ? (
        <>
          ...
          Verifying... {/* Or make loading text a prop */}
        </>
      ) : (
        children
      )}
